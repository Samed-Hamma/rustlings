...
const error_message: &str = "Values out of range.";
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        let error = arr.iter().fold(false, |acc, color| acc || *color < 0 || *color > 255);
        if error {
            return Err(error_message.to_string());
        }
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        let error = slice.iter().fold(false, |acc, color| acc || *color < 0 || *color > 255);
        if error || slice.len() != 3 {
            return Err(error_message.to_string());
        }
